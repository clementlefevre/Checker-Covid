...
def scrap_pdf():
    ...
    response = requests.get(
        "https://covid19.min-saude.pt/wp-content/uploads/2020/04/51_DGS_boletim_20200422.pdf"
    )
    input_bytes.write(response.content)

    extract_text_to_fp(
        input_bytes, output_string, laparams=LAParams(), output_type="html", codec=None
    )
    ...
    ix_hospitalization_icu = [
        (i, t) for i, t in enumerate(list_values) if "NÚMERO  DE CASOS" in t
    ]

    hospitalized = list_values[ix_hospitalization_icu[0][0] + 1]
    icu = list_values[ix_hospitalization_icu[1][0] + 1]

    df_hospi_icu = pd.DataFrame(
        {"key": ["hospitalizations", "icu"], "value": [hospitalized, icu]}
    )
...
def scrap_RKI():
    URL = "https://www.rki.de/DE/Content/InfAZ/N/Neuartiges_Coronavirus/Situationsberichte/2020-04-21-de.pdf?__blob=publicationFile"
    ...
    df = read_pdf("tmp.pdf", pages="all", multiple_tables=True)
