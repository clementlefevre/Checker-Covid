...
def clean_austria():
    ...
    df = pd.read_csv(f'{austria.data_path}/raw/{austria.dt_created}/AllgemeinDaten.csv',sep=';')
    df = df.T
    ...
    df_austria['date']=df_austria[df_austria['original_field_name']=='Timestamp'].iloc[0]['value']
...
def clean_belgium():
    ...
    df_BE_cum_cases['CASES'] = df_BE_cum_cases['CASES'].str.replace("<5","3").astype(int)
    cum_cases = df_BE_cum_cases[~df_BE_cum_cases['NIS5'].isna()]['CASES'].sum()
    df_cases = pd.DataFrame([{'country':'belgium','tub_name':'cases','value':cum_cases}])
...
def clean_france():
    france = COVID('france')
    france.scrapper()
    df_hosp = pd.read_csv(f'{france.data_path}/raw/{france.dt_created}/total_hospitalized.csv')
